    ...
    class AdGroupError(proto.Enum):
        ...
        Values:
            ...
            INVALID_STEP_ID_FOR_VIDEO_ADS_SEQUENCING (20):
                For video ads sequencing, AdGroup ``step_id`` has to use a
                ``step_id`` defined in
                ``campaign.video_campaign_settings.video_ad_sequence``.
            INVALID_AD_GROUP_TYPE_FOR_VIDEO_ADS_SEQUENCING (21):
                For video ads sequencing, AdGroup type has to use a type
                defined in
                ``campaign.video_campaign_settings.video_ad_sequence``.
            DUPLICATE_STEP_ID (22):
                Only one AdGroup is allowed for each step ID
                in video ads sequencing.
            INVALID_VERTICAL_ADS_FORMAT_SETTING (23):
                At least one Vertical Ads format must be
                enabled for a campaign under Travel Ads in
                Search Campaigns.
            VERTICAL_ADS_FORMAT_SETTING_NOT_SUPPORTED_FOR_CAMPAIGNS_WITHOUT_AI_MAX (24):
                AI max setting must be enabled to enable
                Vertical Ads formats for a campaign under Travel
                Ads in Search Campaigns.
            VERTICAL_ADS_FORMAT_SETTING_NOT_SUPPORTED_FOR_CAMPAIGNS_WITHOUT_ENABLED_TRAVEL_FEED (25):
                An enabled travel feed must be linked to
                enable Vertical Ads formats for a campaign under
