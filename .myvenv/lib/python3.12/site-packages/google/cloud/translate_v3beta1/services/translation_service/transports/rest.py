    ...
    * Stripping extraneous information from responses
    ...
    .. code-block:: python
        class MyCustomTranslationServiceInterceptor(TranslationServiceRestInterceptor):
            ...
            def pre_refine_text(self, request, metadata):
                logging.log(f"Received request: {request}")
                return request, metadata

            def post_refine_text(self, response):
                logging.log(f"Received response: {response}")
                return response
    ...
    def post_refine_text(
        ...
        Override in a subclass to read or manipulate the response
        after it is returned by the TranslationService server but before
        it is returned to user code. This `post_refine_text` interceptor runs
        before the `post_refine_text_with_metadata` interceptor.
        """
        return response
    ...
    def post_refine_text_with_metadata(
        ...
        We recommend only using this `post_refine_text_with_metadata`
        interceptor in new development instead of the `post_refine_text` interceptor.
        When both interceptors are used, this `post_refine_text_with_metadata` interceptor runs after the
        `post_refine_text` interceptor. The (possibly modified) response returned by
        `post_refine_text` will be passed to
        `post_refine_text_with_metadata`.
        """
        return response, metadata
    ...
    class _RefineText(
        ...
        def __call__(
            ...
            *,
            ...
            r"""Call the refine text method over HTTP.
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    f"Sending request for google.cloud.translation_v3beta1.TranslationServiceClient.RefineText",
                    extra={
                        "serviceName": "google.cloud.translation.v3beta1.TranslationService",
                        "rpcName": "RefineText",
                        "httpRequest": http_request,
            ...
            # Return the response
            resp = translation_service.RefineTextResponse()
            pb_resp = translation_service.RefineTextResponse.pb(resp)
            ...
            if CLIENT_LOGGING_SUPPORTED and _LOGGER.isEnabledFor(
                ...
                _LOGGER.debug(
                    "Received response for google.cloud.translation_v3beta1.TranslationServiceClient.refine_text",
                    extra={
                        "serviceName": "google.cloud.translation.v3beta1.TranslationService",
                        "rpcName": "RefineText",
                        "metadata": http_response["headers"],
