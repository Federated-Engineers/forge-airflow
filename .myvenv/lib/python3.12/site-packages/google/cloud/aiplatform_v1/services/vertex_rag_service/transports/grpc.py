...
from .base import DEFAULT_CLIENT_INFO, VertexRagServiceTransport
...
class VertexRagServiceGrpcTransport(VertexRagServiceTransport):
    ...
    def ask_contexts(
        ...
        r"""Return a callable for the ask contexts method over gRPC.

        Agentic Retrieval Ask API for RAG.

        Returns:
            Callable[[~.AskContextsRequest],
                    ~.AskContextsResponse]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "ask_contexts" not in self._stubs:
            self._stubs["ask_contexts"] = self._logged_channel.unary_unary(
                "/google.cloud.aiplatform.v1.VertexRagService/AskContexts",
                request_serializer=vertex_rag_service.AskContextsRequest.serialize,
                response_deserializer=vertex_rag_service.AskContextsResponse.deserialize,
            )
        return self._stubs["ask_contexts"]
    ...
    def async_retrieve_contexts(
        ...
        r"""Return a callable for the async retrieve contexts method over gRPC.

        Asynchronous API to retrieves relevant contexts for a
        query.

        Returns:
            Callable[[~.AsyncRetrieveContextsRequest],
                    ~.Operation]:
                A function that, when called, will call the underlying RPC
                on the server.
        """
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "async_retrieve_contexts" not in self._stubs:
            self._stubs["async_retrieve_contexts"] = self._logged_channel.unary_unary(
                "/google.cloud.aiplatform.v1.VertexRagService/AsyncRetrieveContexts",
                request_serializer=vertex_rag_service.AsyncRetrieveContextsRequest.serialize,
                response_deserializer=operations_pb2.Operation.FromString,
            )
        return self._stubs["async_retrieve_contexts"]
