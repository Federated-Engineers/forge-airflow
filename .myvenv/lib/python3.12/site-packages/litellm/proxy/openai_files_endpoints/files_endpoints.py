...
from litellm.proxy.common_utils.http_parsing_utils import (
    _read_request_body, extract_nested_form_metadata)
...
from litellm.types.llms.openai import (CREATE_FILE_REQUESTS_PURPOSE,
                                       FileExpiresAfter, OpenAIFileObject,
                                       OpenAIFilesPurpose)
...
async def create_file(  # noqa: PLR0915
    ...
    try:
        ...
        if enforced_file_expiry is not None:
            if (
                "anchor" not in enforced_file_expiry
                or "seconds" not in enforced_file_expiry
            ):
                raise HTTPException(
                    status_code=500,
                    detail={
