...
from awswrangler._utils import (copy_df_shallow, ensure_worker_or_thread_count,
                                is_pandas_frame, split_pandas_frame,
                                table_refs_to_df)
...
from awswrangler.distributed.ray.s3._read_orc import \
    _read_orc_metadata_file_distributed
...
def register_ray() -> None:
    ...
    if memory_format.get() == MemoryFormatEnum.MODIN:
        from awswrangler.distributed.ray.modin._data_types import \
            pyarrow_types_from_pandas_distributed
        from awswrangler.distributed.ray.modin._utils import (
            _arrow_refs_to_df, _copy_modin_df_shallow,
            _is_pandas_or_modin_frame, _split_modin_frame)
        from awswrangler.distributed.ray.modin.s3._read_orc import \
            _read_orc_distributed
        from awswrangler.distributed.ray.modin.s3._read_parquet import \
            _read_parquet_distributed
        from awswrangler.distributed.ray.modin.s3._read_text import \
            _read_text_distributed
        from awswrangler.distributed.ray.modin.s3._write_dataset import (
            _to_buckets_distributed, _to_partitions_distributed)
        from awswrangler.distributed.ray.modin.s3._write_orc import \
            _to_orc_distributed
        from awswrangler.distributed.ray.modin.s3._write_parquet import \
            _to_parquet_distributed
        from awswrangler.distributed.ray.modin.s3._write_text import \
            _to_text_distributed
